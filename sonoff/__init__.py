...
class Sonoff:
    def __init__(self, hass, config):
        ...
        self._grace_period = timedelta(
            seconds=config.get(DOMAIN, {}).get(CONF_GRACE_PERIOD, "")
        )
    ...
    def do_login(self):
        ...
        self._model = "iPhone" + random.choice(
            [
                "6,1",
                "6,2",
                "7,1",
        ...
        self._appVersion = random.choice(
            [
                "3.5.3",
                "3.5.4",
                "3.5.6",
    ...
    def set_wshost(self):
        r = requests.post(
            "https://%s-disp.coolkit.cc:8080/dispatch/app" % self._api_region,
            headers=self._headers,
        )
        resp = r.json()
    ...
    async def state_listener(self, event):
        ...
        for idxd, dev in enumerate(self._devices):
            if dev["deviceid"] == device["deviceid"]:
                if outlet is not None:
                    self._devices[idxd]["params"]["switches"][outlet][
                        "switch"
                    ] = new_state
                else:
    ...
    def init_websocket(self):
        ...
        while True:
            ...
            self._ws = WebsocketListener(
                sonoff=self, on_message=self.on_message, on_error=self.on_error
            )
                        ...
                        if "switches" in data["params"]:
                            for switch in data["params"]["switches"]:
                                self.set_entity_state(
                                    data["deviceid"], switch["switch"], switch["outlet"]
                                )
                        else:
                            self.set_entity_state(
                                data["deviceid"], data["params"]["switch"]
                            )
    ...
    def on_error(self, *args):
        error = args[
            -1
        ]  # to accomodate the case when the function receives 2 or 3 args
        _LOGGER.error("websocket error: %s" % str(error))

    def is_grace_period(self):
        grace_time_elapsed = self._skipped_login * int(
            self.get_scan_interval().total_seconds()
        )
    ...
    def get_outlets(self, device):
        ...
        if (
            device["uiid"] in uiid_to_name.keys()
            and uiid_to_name[device["uiid"]] in name_to_outlets.keys()
        ):
    ...
    def write_debug(self, data, type="", new=False):
        ...
        if (
            self._sonoff_debug
            and self._hass.states.get("switch.sonoff_debug")
            and self._hass.states.is_state("switch.sonoff_debug", "on")
        ):
            ...
            self._sonoff_debug
            and len(self._sonoff_debug_log)
            and self._hass.states.get("switch.sonoff_debug")
            and self._hass.states.is_state("switch.sonoff_debug", "off")
        ):
            ...
            self._hass.components.persistent_notification.async_create(
                str(b"".join(self._sonoff_debug_log), "utf-8"), "Sonoff debug"
            )
            self._sonoff_debug_log = []
    ...
    def clean_data(self, data):
        ...
        data = re.sub(
            r'"\w{2}:\w{2}:\w{2}:\w{2}:\w{2}:\w{2}"', '"xx:xx:xx:xx:xx:xx"', data
        )
        data = re.sub(
...
class WebsocketListener(threading.Thread, websocket.WebSocketApp):
    def __init__(self, sonoff, on_message=None, on_error=None):
        ...
        websocket.WebSocketApp.__init__(
            self,
            "wss://{}:8080/api/ws".format(self._sonoff.get_wshost()),
...
class SonoffDevice(Entity):
    ...
    def get_state(self):
        ...
        # TH10/TH16
        if (
            "currentHumidity" in device["params"]
            and device["params"]["currentHumidity"] != "unavailable"
        ):
            self._attributes["humidity"] = device["params"]["currentHumidity"]
        if (
            "currentTemperature" in device["params"]
            and device["params"]["currentTemperature"] != "unavailable"
        ):
            self._attributes["temperature"] = device["params"]["currentTemperature"]

        if (
            "humidity" in device["params"]
            and device["params"]["humidity"] != "unavailable"
        ):
            self._attributes["humidity"] = device["params"]["humidity"]
        if (
            "temperature" in device["params"]
            and device["params"]["temperature"] != "unavailable"
        ):
        ...
        if self._outlet is not None:
            return (
                device["params"]["switches"][self._outlet]["switch"] == "on"
                if device
                else False
            )
