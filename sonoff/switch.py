...
from custom_components.sonoff import DOMAIN as SONOFF_DOMAIN
from custom_components.sonoff import SonoffDevice
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    for device in hass.data[SONOFF_DOMAIN].get_devices(force_update=True):
        ...
        if outlets_number is None:  # fallback to whatever the device might have
            if (
                "switches" in device["params"]
            ):  # the device has multiple switches, split them by outlets
            ...
            outlets_number > 1
        ):  # the device has multiple switches, split them by available outlets
        ...
        # normal device = Sonoff Basic (and alike)
            ...
            "switch" in device["params"] or "state" in device["params"]
        ):  # ignore devices like Sonoff RF bridge:
            entity = SonoffSwitch(hass, device)
            entities.append(entity)
...
class SonoffSwitch(SonoffDevice, SwitchDevice):
    ...
    def __init__(self, hass, device, outlet=None):
        ...
        self._outlet = outlet
        self._name = "{}{}".format(
            device["name"], "" if outlet is None else " " + str(outlet + 1)
        )
        ...
        if outlet is None:
            ...
            if "tags" in device and "ck_channel_name" in device["tags"]:
                if (
                    str(outlet) in device["tags"]["ck_channel_name"].keys()
                    and device["tags"]["ck_channel_name"][str(outlet)]
                ):
                    self._name = "{} - {}".format(
                        device["name"], device["tags"]["ck_channel_name"][str(outlet)]
                    )

                    self._attributes["outlet_name"] = device["tags"]["ck_channel_name"][
                        str(outlet)
                    ]
                else:
                    self._name = "{} {}".format(
                        device["name"], ("CH %s" % str(outlet + 1))
                    )
            else:
    ...
    def turn_on(self, **kwargs):
        ...
        self._hass.bus.async_fire(
            "sonoff_state",
            {"state": True, "deviceid": self._deviceid, "outlet": self._outlet},
        )
    ...
    def turn_off(self, **kwargs):
        ...
        self._hass.bus.async_fire(
            "sonoff_state",
            {"state": False, "deviceid": self._deviceid, "outlet": self._outlet},
        )
        self.async_schedule_update_ha_state()
...
class SonoffDebugSwitch(SwitchDevice):
    ...
    def turn_off(self, **kwargs):
        ...
        self._hass.data[SONOFF_DOMAIN].write_debug(
            "{}"
        )  # send dummy data to trigger the notification
