...
if not sys.argv[1].startswith("-") and not sys.argv[2].startswith("-"):
    ...
    parser.add_argument(
        "-u", "--username", help="email/phone number used to login in eWeLink app"
    )
...
def do_login():
    ...
    try:
        ...
        hex_dig = hmac.new(
            decryptedAppSecret, json.dumps(app_details), digestmod=hashlib.sha256
        ).digest()
    ...
    r = requests.post(
        "https://%s-api.coolkit.cc:8080/api/user/login" % api_region,
        headers=headers,
        json=app_details,
    )
    resp = r.json()
